...
class SeleksiController {
    ...
    /**
     * GET findSeleksiTerbuka
     */
    static async findSeleksiTerbuka(req, res) {
        try {
            const { id } = req.params;
            const data_exec = await SeleksiService.findSeleksiTerbuka();
            return res.status(200).json({
                message: 'Data detail',
                data: data_exec
            });
    ...
    static async destroy(req, res) {
        try {
            const { id } = req.params;
            const seleksi_id = req.seleksi_id;
            const data_exec = await SeleksiService.destroy(id,seleksi_id);
            return res.status(200).json({
